...
//     // Default chart for summary/detailed reports
...
// return {
//   labels: data.map(item => item.period_label || item.service_name || item.stylist_name),
//   datasets: [
//     {
//       label: 'Revenue',
//       data: data.map(item => {
...
//             data: data.map(item => item.moving_avg_7),
...
//       return {
//         labels: data.map(item => item.stylist_name),
//         datasets: [
//           {
//             label: 'Revenue',
...
//     const { current_total, previous_total, yoy_total } = previewData.metrics;
//     const prevPeriodChange = ((current_total - previous_total) / previous_total) * 100;
//     const yoyChange = ((current_total - yoy_total) / yoy_total) * 100;
...
//                   </TableHead>
//                   <TableBody>
//                     {(previewData.rows || previewData.current_period || []).slice(0, 50).map((row) => (
//                       <TableRow key={`${row.period_key || row.stylist_name || Math.random()}-${row.service_name || ''}`}>
//                         {reportConfig.reportType === 'comparison' ? (
//                           <>
//                             <TableCell>{row.period_label}</TableCell>
//                             <TableCell align="right">{formatCurrency(row.total_revenue)}</TableCell>
//                             <TableCell align="right">
//                               {formatCurrency(previewData.previous_period.find(p => p.period_key === row.period_key)?.total_revenue || 0)}
//                             </TableCell>
//                             <TableCell align="right">
//                               <Chip 
//                                 label={`${(
...
//                         ) : reportConfig.reportType === 'stylist' ? (
//                           <>
//                             <TableCell>{row.stylist_name}</TableCell>
//                             <TableCell align="right">{formatCurrency(row.total_revenue)}</TableCell>
...
//                         ) : (
//                           <>
//                             <TableCell>{row.period_label || row.service_name || ''}</TableCell>
...
//                               )}
//                             </TableCell>
//                             <TableCell align="right">{row.unique_clients || '-'}</TableCell>
//                           </>
//                         )}
//                       </TableRow>
//                     ))}
...
//                 Key Insights
//               </Typography>
//               {/* This would be enhanced with actual insights generation */}
...
//   if (config.reportType === 'comparison') {
...
//     const prevChange = ((currentTotal - prevTotal) / Math.max(prevTotal, 1)) * 100;
//     const yoyChange = ((currentTotal - yoyTotal) / Math.max(yoyTotal, 1)) * 100;
...
//     const totalChange = ((lastRevenue - firstRevenue) / Math.max(firstRevenue, 1)) * 100;
