...
const debugLog = (label, data, level = 'log') => {
...
export default function AppointmentForm({
  ...
  const [form, setForm] = useState(() => ({
    ...
    serviceIds: Array.isArray(initialData.serviceIds) ? initialData.serviceIds : [],
    stylists: Array.isArray(initialData.stylists) ? initialData.stylists : [],
  ...
  const [timeslotDebugInfo, setTimeslotDebugInfo] = useState('');

  // Enhanced initialization with service validation and debugging
  useEffect(() => {
    debugLog('Initialization triggered', { 
      open, 
      initialized, 
      initialData, 
    ...
    if (open && !initialized) {
      // Debug incoming data
      debugLog('Services data structure', services);
      debugLog('Initial data structure', initialData);
      ...
      if (initialData.customer_ID && customers.length > 0) {
        ...
        debugLog('Customer found', customerObj);
      }

      // Services initialization - more robust matching
      let serviceObjs = [];
      if (Array.isArray(initialData.services) && initialData.services.length > 0) {
        debugLog('Initializing from service names', initialData.services);
        serviceObjs = initialData.services
          .map(serviceName => {
            ...
            if (!serviceObj) {
              console.warn(`Service not found in available services: "${serviceName}"`);
            }
            return serviceObj;
          })
          .filter(Boolean);
      } else if (Array.isArray(initialData.serviceIds) && initialData.serviceIds.length > 0) {
        debugLog('Initializing from service IDs', initialData.serviceIds);
        serviceObjs = initialData.serviceIds
          .map(serviceId => {
            const serviceObj = services.find(s => String(s.service_ID) === String(serviceId));
            if (!serviceObj) {
              console.warn(`Service ID not found: ${serviceId}`);
            }
            return serviceObj;
          })
          .filter(Boolean);
      }
      debugLog('Initialized services', serviceObjs);
...
// Replace your current timeslot fetching useEffect with this:
useEffect(() => {
  ...
  setDebugInfo(debugMessages.join('\n'));
  fetchTimeSlots(form.appointment_date, selectedStylists.map(s => s.stylist_ID), serviceIds);
}, [form.appointment_date, selectedServices, selectedStylists]);

  // Debug selected services changes
  useEffect(() => {
    debugLog('Selected services state changed', {
      selectedServices,
      formServices: form.services,
      formServiceIds: form.serviceIds
    });
  }, [selectedServices]);
  ...
  const handleSubmit = (e) => {
    ...
    if (!validateForm()) {
      debugLog('Form validation failed', errors, 'error');
      return;
    }
    ...
    debugLog('Submitting form with payload', payload);
    onSubmit(payload);
  };
  ...
  const renderTimeSlots = () => {
    // Debug logging
    console.log('Rendering timeslots:', {
      loading: loadingTimeSlots,
      slots: availableTimeSlots,
      count: availableTimeSlots?.length,
      formTime: form.appointment_time
    });
