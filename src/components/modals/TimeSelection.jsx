...
//       // Ensure services is an array or handle it as a single object
//       const servicesToProcess = Array.isArray(services) ? services : (services ? [services] : []);
      
//       if (servicesToProcess.length === 0) return 60; // Default duration if no services
//       return servicesToProcess.reduce((total, service) => total + (service.time_duration || 0), 0);
//     }, [services]);
...
//       // First day of month in the week
...
//         // If there are available days, select the first future available date
...
//           } else {
//             // If no future dates, just select the first available
...
//       try {
//         // Ensure services is an array or convert it to array if it's a single object
//         const servicesToAdd = Array.isArray(services) ? services : (services ? [services] : []);
        
//         if (servicesToAdd.length === 0) {
//           console.error("No services available to add to cart");
//           return;
//         }
...
//             console.error('Missing required fields:', cartData);
...
//     // Get the month and year string for display
//     const getMonthYear = () => {
//       return currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
//     };

//     if (showCart) {
//       // Ensure services is properly passed to CartModal
//       const servicesToPass = Array.isArray(services) ? services : (services ? [services] : []);
...
const TimeSelectionModal = forwardRef(
  ({ services, customerId, selectedStylist, onBack, onClose }, ref) => {
    ...
    const normalizedServices = useCallback(() => {
      // If services is falsy (null/undefined), return empty array
      if (!services) return [];
      // If services is already an array, return it
      if (Array.isArray(services)) return services;
      // If services is a single object, wrap it in an array
      return [services];
    }, [services]);
    ...
    const getTotalServiceDuration = useCallback(() => {
      ...
      if (servicesList.length === 0) return 60; // Default duration if no services
      
      // Extract time duration, handle different property names
      return servicesList.reduce((total, service) => {
        // Check for different possible duration properties
        const duration = service.time_duration || 
                         service.duration || 
                         (typeof service.duration === 'string' ? parseInt(service.duration, 10) : 0);
        
        return total + (isNaN(duration) ? 60 : duration); // Default to 60 if invalid
      }, 0);
    }, [normalizedServices]);
...
//       // Auto-select first time slot if available and not too soon
//       if (slots.length > 0) {
//         const today = getTodayDateString();
//         if (dateString === today && cutoffTime) {
//           // For today, find first slot that's after cutoff
//           const firstValidSlot = slots.find(slot => {
//             const slotTime = new Date(`${dateString}T${slot}:00`);
...
//       initializeCalendar();
//     }, [currentMonth, fetchAvailableDatesForMonth, buildCalendarDays, cutoffTime]);
...
//       // Additional validation for 12-hour cutoff
//       const today = getTodayDateString();
//       if (selectedDate === today && cutoffTime) {
//         const selectedDateTime = new Date(`${selectedDate}T${selectedTime}:00`);
//         if (selectedDateTime < cutoffTime) {
//           setError("Appointments must be booked at least 12 hours in advance.");
//           return;
//         }
//       }
...
//                   {timeSlots.map((time, index) => {
//                     const today = getTodayDateString();
//                     const isTooSoon = selectedDate === today && cutoffTime 
//                       ? new Date(`${selectedDate}T${time}:00`) < cutoffTime
//                       : false;
...
//                         <Paper
//                           elevation={0}
//                           onClick={() => !isTooSoon && setSelectedTime(time)}
//                           sx={{
//                             p: 2,
//                             textAlign: "center",
//                             cursor: isTooSoon ? "not-allowed" : "pointer",
//                             borderRadius: "8px",
//                             height: '100%',
//                             display: 'flex',
//                             alignItems: 'center',
